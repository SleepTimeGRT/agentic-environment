...
# Part 6: Agentic Coding 방법론 (부록)

> 도구(cmux, hooks)와 방법론(Plan-first, TDD, Closed-loop)은 상호배제가 아니라 **서로를 강화**.

## 방법론 × 도구 매트릭스

| 방법론 | 도구 | 시너지 |
|--------|------|--------|
| Plan → Annotate → Implement | plan.md + Plan Mode | plan이 context compaction에서 살아남는 영속 기억 |
| Red/Green TDD | PostToolUse 훅 (vitest) | 에이전트가 테스트 실패→수정→재실행 자동 루프 |
| Closed-Loop | lint + tsc + test 훅 체인 | 사람이 루프에서 빠짐 |
| Independent Review | cmux 별도 워크스페이스 | 구현/리뷰 에이전트 동시 모니터링 |
| Parallel Exploration | cmux + worktree | 같은 스펙, 다른 접근 — 추가 시간 비용 0 |
| Context Engineering | SessionStart 훅 + CLAUDE.md + memory | 매 세션 올바른 컨텍스트 자동 로딩 |

## 코어 테크닉

### 1. Research → Plan → Annotate → Implement (Boris Tane)

1. **Research**: "관련 파일을 깊이 읽고 research.md에 정리해" → 파일로 저장 (compaction 생존)
2. **Plan**: "구현 계획을 plan.md에 작성해. 아직 구현하지 마"
3. **Annotate**: 사용자가 plan.md를 직접 편집 → "여기 X라고 가정했는데 실제론 Y야" → 1-6회 반복
4. **Implement**: "plan.md의 모든 항목을 구현해. 중간에 멈추지 마. 타입체크를 계속 돌려"

**도구 시너지**: plan.md가 Claude의 Plan Mode보다 나은 이유 — 사람이 직접 편집 가능 + compaction 생존.

### 2. Red/Green TDD

프롬프트 4단어: **"Use red/green TDD."** 테스트가 실행 가능한 스펙 역할.
**도구 시너지**: PostToolUse에 vitest → 에이전트가 자동 Red→Green 루프.

### 3. Closed-Loop

에이전트가 사람 없이 오류 발견→수정→재검증하는 자율 루프.
전제: 검증 도구가 deterministic이어야 함 (flaky 테스트 → 무한 루프).
**도구 시너지**: PostToolUse 훅 체인 (format → lint → tsc → test).

### 4. Independent Review Agent

구현 에이전트와 다른 세션에서 리뷰. 구현 에이전트는 anchoring bias 있음.
프롬프트: "문제만 찾아. 코드가 뭘 하는지 설명하지 마."
**도구 시너지**: cmux 사이드바에서 구현/리뷰 동시 모니터링.

### 5. Parallel Exploration

같은 스펙에 다른 접근법으로 두 에이전트 동시 실행 → 비교 → 나은 쪽 선택.
추가 시간 비용 = 0.
**도구 시너지**: cmux + worktree.

### 6. Context Engineering

CLAUDE.md, plan.md, memory/는 문서가 아니라 **에이전트의 작업 환경 설계**.
4가지 연산: Write (파일에 기록) → Select (관련 것만 로딩) → Compress (/compact) → Isolate (서브에이전트로 격리).
...
| 단일 에이전트에 전체 위임 | 역할별 에이전트 분리 |

---

# Part 7: 하네스 엔지니어링 (Harness Engineering)
...
OpenAI Codex 팀이 3명으로 100만 줄을 작성하며 용어를 대중화 (2026-02). Anthropic, Thoughtworks(Martin Fowler), Philipp Schmid 등이 프레임워크를 발전시키고 있음.

승마 비유: 모델이 말이라면, 하네스는 그 힘을 생산적으로 방향짓는 장비. 엔지니어는 방향을 제시.

## 7.2 Prompt → Context → Harness: 세 시대
...
| **사람의 역할** | 작성자 | 아키텍트 | 환경 설계자 |
| **실패 대응** | 프롬프트 재작성 | 컨텍스트 데이터 점검 | 역량 또는 가드레일 추가 |

비유 (SoftmaxData): Prompt Engineering은 전기기사에게 말하는 법, Context Engineering은 청사진을 건네는 것, Harness Engineering은 여러 작업자가 부딪히지 않도록 **현장 전체를 설계**하는 것.

컴퓨팅 비유 (Philipp Schmid): 모델 = CPU, 컨텍스트 윈도우 = RAM, 하네스 = OS, 에이전트 = 애플리케이션.

## 7.3 하네스의 5가지 기둥

1. **도구 오케스트레이션** — 에이전트가 접근할 수 있는 도구, 호출 방법, 권한 정의
2. **가드레일과 안전 제약** — 권한 경계의 결정론적 규칙, 검증 체크포인트, 아키텍처 강제, 속도 제한
3. **에러 복구와 피드백 루프** — 재시도 로직, 자체 검증 루프, 롤백 메커니즘, 루프 감지
4. **관측성 (Observability)** — 에이전트 행동 로깅, 토큰 사용 추적, 의사결정 기록, 이상 탐지
5. **Human-in-the-Loop 체크포인트** — 고위험 순간의 전략적 승인 게이트

### 이 레포에서의 매핑

| 기둥 | 이 레포의 구현 |
|------|--------------|
| 도구 오케스트레이션 | `settings.json` — plugins, MCP servers |
| 가드레일 | `deny` 규칙 (`.env`, `rm -rf`), `settings.team.json` (`--force` 방지) |
| 피드백 루프 | PostToolUse 훅 체인 (`format → lint → tsc`) |
| 관측성 | `session-context.sh`, statusLine 설정 (ykdojo 접근법) |
| HitL 체크포인트 | Auto Mode (위험 작업만 차단), plan.md 승인 |

## 7.4 OpenAI Codex 팀의 5원칙

3명이 5개월간 100만 줄, 1,500 PR, 수동 타이핑 0줄로 달성한 원칙:

**1. 에이전트가 볼 수 없으면 존재하지 않는다**
모든 의사결정을 레포 안 문서로 밀어넣기. "ExecPlans" (PLANS.md에 자기완결적 설계문서). 컨텍스트가 완전하면 7시간+ 연속 작업 가능.

**2. "왜 실패하나" 대신 "어떤 역량이 빠졌나"를 묻기**
문제를 프롬프트 결함이 아닌 환경 결핍으로 재프레이밍. 프롬프트를 조정하는 대신 커스텀 도구를 빌드. "안정적이고 잘 문서화된 '지루한 기술'이 에이전트에게 유리."

**3. 문서화보다 기계적 강제**
규칙을 산문이 아닌 코드로 강제. 레이어 구조 위반을 커스텀 린터로 빌드 실패 처리. "린터 자체도 Codex가 작성."

**4. 에이전트에게 눈을 줘라**
관측성과 시각적 도구 연결. Chrome DevTools Protocol으로 DOM 스냅샷, Victoria Logs/Metrics 통합. 일부 에이전트 런이 6시간+ 집중 유지.

**5. 매뉴얼이 아닌 지도**
간결한 아키텍처 오버뷰 제공, 상세 문서 아님. ARCHITECTURE.md는 거의 변하지 않는 구조만. "아키텍처 불변성은 종종 '여기에 X가 없다'로 표현 — 반직관적이지만 효과적."

## 7.5 실증 데이터: 하네스가 모델보다 중요한 증거

| 실험 | 결과 |
|------|------|
| **Hashline** — 편집 포맷만 변경 (라인 해싱 도입) | 동일 모델, 성능 6.7% → **68.3%** |
| **LangChain** — 하네스 개선만 적용 | 벤치마크 52.8% → **66.5%**, 순위 ~30위 → ~5위 |
| **OpenAI Codex** — 하네스 원칙 적용 | 3명 × 5개월 = 100만 줄, **~10x 생산성** |

> 모델 가중치 변경 없이 하네스만으로 10배 성능 차이가 발생한다는 실증.

## 7.6 Anthropic의 멀티 에이전트 하네스 설계

GAN에서 영감받은 장기 실행 아키텍처:

- **Planner Agent**: 간결한 프롬프트를 상세 스펙으로 확장
- **Generator Agent**: 스프린트 단위로 점진적 구현
- **Evaluator Agent**: Playwright로 실행 중인 앱을 테스트, 외부 피드백 제공

핵심 통찰: "평가를 생성에서 분리하는 것이, 생성자가 자기 작업을 비판하게 만드는 것보다 훨씬 다루기 쉽다." 장기 실행 태스크에서 컨텍스트 요약(compaction)보다 구조화된 핸드오프가 나음.

## 7.7 이 레포와의 관계

이 레포(`agentic-environment`)는 본질적으로 **하네스 엔지니어링 스타터 킷**:

- `bootstrap.sh` = 하네스 설치 자동화
- `configs/settings.json` = 기둥 1-2 (도구 오케스트레이션 + 가드레일)
- `templates/settings.typescript.json` = 기둥 3 (피드백 루프)
- statusLine 설정 = 기둥 4 (관측성)
- `scripts/session-context.sh` = 기둥 1 (컨텍스트 자동 주입)
- `templates/CLAUDE.md` = OpenAI 원칙 3 ("기계적 강제" — 실패를 규칙으로 전환)
